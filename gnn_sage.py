"""
cresci-2017 baseline pipeline — Step 4: GraphSAGE GNN
======================================================
Architecture: GraphSAGE (Hamilton et al. 2017)
  - Chosen over GCN because it handles isolated nodes gracefully via self-loops
    and mean aggregation — GCN's symmetric normalisation collapses on degree-0 nodes
  - 3 layers: 256 → 128 → 64, ReLU + dropout, binary output head
...
Training:
  ...
  - Trains on full graph each fold — inductive (SAGEConv) so val nodes
    are seen only at inference, not during message passing aggregation

Also fixes the genuine-class F1=0 reporting issue from Step 3 by printing
a full sklearn classification_report with per-class breakdown.
"""
...
class BotSAGE(nn.Module):
    ...
    3-layer GraphSAGE with:
      - mean aggregation (robust to varying neighbourhood sizes)
      - BatchNorm after each conv (stabilises training on mixed-scale features)
    ...
    def forward(self, x, edge_index):
        ...
        return self.head(x).squeeze(-1)   # logits, shape [N]
...
edges_df = graph_data["edges"]   # columns: retweeter_id, original_author_id, weight

# Build node index mapping: user_id str → integer index (row in df)
...
# Clip extreme outliers (99.9th pct) — same as RF
...
print(f"\nTraining GraphSAGE ({EPOCHS} epochs max, patience={PATIENCE})...")
...
for fold, (tr_idx, va_idx) in enumerate(CV.split(X_raw, y_all)):
    ...
    # Class-weighted positive weight for BCE
    n_neg = (y_all[tr_idx] == 0).sum()
    n_pos = (y_all[tr_idx] == 1).sum()
    pos_weight = torch.tensor([n_neg / n_pos], dtype=torch.float32).to(DEVICE)

    model     = BotSAGE(in_dim=len(FEATURES), hidden=HIDDEN, dropout=DROPOUT).to(DEVICE)
...
print("="*60)
print(f"  F1 (macro): {sage_results['f1_macro']['mean']:.4f} ± {sage_results['f1_macro']['std']:.4f}")
print(f"  AUC-ROC:    {sage_results['auc_roc']['mean']:.4f} ± {sage_results['auc_roc']['std']:.4f}")
print(f"  Accuracy:   {sage_results['accuracy']['mean']:.4f}")
...
# GraphSAGE row
sg_f1  = sage_results["f1_macro"]
sg_auc = sage_results["auc_roc"]
sg_acc = sage_results["accuracy"]
print(f"  {'GraphSAGE':20s}  {sg_f1['mean']:.4f}±{sg_f1['std']:.4f}  "
      f"{sg_auc['mean']:.4f}±{sg_auc['std']:.4f}  {sg_acc['mean']:.4f}")

print("\n  Per-category F1 comparison:")
all_cats = sorted(set(bot_types))
print(f"  {'category':25s}  {'RF-node':>10s}  {'RF+graph':>10s}  {'GraphSAGE':>10s}")
print("  " + "-" * 65)
for cat in all_cats:
    rf_n  = rf_results["node_only"]["category_f1"].get(cat, 0.0)
    rf_g  = rf_results["node+graph"]["category_f1"].get(cat, 0.0)
    sg    = cat_f1_sage.get(cat, 0.0)
    flag  = "  ← best" if sg == max(rf_n, rf_g, sg) and sg > rf_g + 0.001 else ""
    print(f"  {cat:25s}  {rf_n:10.4f}  {rf_g:10.4f}  {sg:10.4f}{flag}")

# Save updated results
rf_results["graphsage"] = {
    "metrics": {k: {"mean": float(v["mean"]), "std": float(v.get("std", 0))}
                for k, v in sage_results.items()},
    "category_f1": cat_f1_sage,
}
...
print("Next: your new framework goes here — compare against these numbers.")
