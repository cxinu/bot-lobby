"""
cresci-2017 baseline pipeline — GCN baseline
=============================================
Architecture: GCN (Kipf & Welling 2017)
  - GCNConv with normalize=True (symmetric normalisation + internal self-loops)
...
class BotGCN(nn.Module):
    """
    3-layer GCN with:
      - symmetric normalisation (normalize=True)
...
print(f"  Nodes: {len(df):,}  |  Edges (no self-loops): {edge_index.shape[1]:,}")
...
print(f"\nTraining GCN ({EPOCHS} epochs max, patience={PATIENCE})...")
...
for fold, (tr_idx, va_idx) in enumerate(CV.split(X_raw, y_all)):
    ...
    model     = BotGCN(in_dim=len(FEATURES), hidden=HIDDEN, dropout=DROPOUT).to(DEVICE)
...
print("GCN RESULTS")
print("="*60)
print(f"  F1 (macro): {gcn_results['f1_macro']['mean']:.4f} ± {gcn_results['f1_macro']['std']:.4f}")
print(f"  AUC-ROC:    {gcn_results['auc_roc']['mean']:.4f} ± {gcn_results['auc_roc']['std']:.4f}")
print(f"  Accuracy:   {gcn_results['accuracy']['mean']:.4f}")
...
# GCN row
gc_f1 = gcn_results["f1_macro"]
gc_au = gcn_results["auc_roc"]
gc_ac = gcn_results["accuracy"]
...
print(f"  {'category':25s}  {'RF-node':>10s}  {'RF+graph':>10s}  {'GraphSAGE':>10s}  {'GCN':>10s}")
print("  " + "-" * 80)
for cat in all_cats:
    ...
    gc   = cat_f1_gcn.get(cat, 0.0)
    print(f"  {cat:25s}  {rf_n:10.4f}  {rf_g:10.4f}  {sg:10.4f}  {gc:10.4f}")

# Save updated results
rf_results["gcn"] = {
    "metrics": {k: {"mean": float(v["mean"]), "std": float(v.get("std", 0))}
                for k, v in gcn_results.items()},
    "category_f1": cat_f1_gcn,
}
